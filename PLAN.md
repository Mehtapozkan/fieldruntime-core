...
## Current bounded D13 handoff: synthetic comparison readiness
...
[D12-A / Accepted D-037](docs/architecture/d12-bounded-preparation-worker.md) merged normally
in PR #37 at `7b3b3e125591212a6392504298d9b467f1e2f464`, preserving reviewed
`7ac9f9db` and the owner approval at `0812e004` after required CI. D12-B merged in PR #38 at `7db84da759a6a66ed713e5a51bd8f06bfa87e951`
from reviewed `5100ef8d` with required checks and resolved findings: one fixed zero-model preparation worker and separate task,
correction/evaluation and bounded synthetic proof history. [Walkthrough and limits](docs/guides/synthetic-preparation-worker.md).
No consequential-rule activation, real data, release or deployment.
...
| D12 — One Useful Replaceable Worker                            | Merged                                                                             | Evidence gathering, reconciliation and drafts within reviewed read/propose scope; bounded execution, independent proof and minimum manual reviewed correction capture                        | API-only scoped work/denial/abstention/exact retry/result inspection; replace worker without losing history; code-only and human-approved AI controls; no unsupported closure         |
| D13 — 25-Case Challenge + Customer Proof                       | In progress; result API/UI and fake investigation in review; live model unapproved | Complete one imported-dispute path through supported result, minimum Workbench controls and a separately approved bounded investigation task; Operating Capacity Map and customer comparison | Synthetic end-to-end evidence is separate from customer quality, effort, outcomes and repeated demand; five proof measures stay distinct; paid continuation is an experiment, not PMF |
...
6. **D12-A/B merged:** explicit worker-capable pack v2,
   fixed deterministic worker/profile and one supporting journal. W1–W10 cover
   scoped checklist/reconciliation/unsent requests, exact start/terminal fences,
   current permission, retries/interruption, task/correction/evaluation review and
...
is merged under Accepted D-037; D13 comparisons include failed/open work and all attributable
...
or a separately approved bounded change is recorded. The single D13 investigation task is distinct from broad D15 routing. Accepted
D-040 permits the bounded fake-provider implementation; live activation, inference
spending and customer-data processing still require separate approval.

## D13 first bounded delivery

The [read-only report/rehearsal](docs/guides/challenge-report.md) uses existing
...
**Preserved API dependency, for review in PR #41:** [Accepted D-039](docs/architecture/d13-imported-dispute-result.md),
approved at `1f387d470a90dfe41df69335732b3c48c831750a`, implements the single synthetic
no-adjustment path with exact D6 authority, original proof/terms, reported disposition,
independent observation and separate business acceptance/reopening. One additive
result journal and migration 0011 preserve old histories; R stays separate from O/C/S.
[Executable API and BR1–BR8](docs/guides/synthetic-dispute-result.md). This PR is stacked
on open PR #40 and is to remain open for review. Existing Challenge report versions,
five measures and D-038 preparation stay intact. No model implementation, financial effect, customer processing or complete Case closure is included in that API dependency.

**Current Workbench implementation, for review:** a separate branch from PR #41 at
`4a1f536f` connects the existing API through explicit result controls, reconciled
record evidence, independent checking, separate acceptance and recovery. PRs #40/#41
and this Workbench remain open/unmerged; no merge is authorized by this assignment.
[Walkthrough and boundary](docs/guides/dispute-result-workbench.md). D-038 acceptance
evidence, migrations and all prior runtime rules remain unchanged. The separately
[Accepted D-040 investigation task](docs/architecture/d13-bounded-investigation-task.md)
is implemented with a fake-only adapter in a separate PR stacked on #42. Live
activation and spending remain unavailable. Stop at that implementation handoff.

### Remaining D13 MVP sequence

This September 10 planning reconciliation replaces the unsent next-step sequencing
drafts, preserves the canonical requirements and does not itself establish implemented behavior. D-039 approval is recorded
separately at the exact reviewed head; its API delivery is stated above. Keep ECC, the existing Case runtime and the already identified
invoice-dispute entry. General payment matching, bank connectivity and cash application
are not prerequisites for this MVP. The product goal is one traceable record from
investigation to a supported result and an informed next improvement decision.

| Order                                                   | Bounded delivery                                                                                                                                                                                                                            | Exit evidence and dependency                                                                                                                                                                                                                                        |
| ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Now                                                     | Finish the minimum same-dispute Workbench on the open D-039 API dependency; preserve PR #40 and stop at the new open Workbench PR.                                                                                                          | Preserve reviewed D-038 evidence. D-039 and the model/provider/data boundary are separate decisions; neither approval is inferred from this plan.                                                                                                                   |
| Next                                                    | Review D-039 API evidence and dependency before any normal protected merge. Accepted D-040 implementation is in review with fake responses only; separately approve any live synthetic evaluation.                                          | BR1–BR8: same-record proof, exact authority, reported disposition, independent observation, separate business acceptance, remaining commitments and reversal. No model dependency or complete Case closure.                                                         |
| Immediately after the API                               | Review the implemented minimum result controls and reconciled progress in the existing Workbench. Review the separate bounded investigation implementation; retain the deterministic default until a separately approved evaluation passes. | Use the same result contracts, exact receipts and source associations. Demonstrate useful work, material uncertainty, next responsible role/action and current result state. Keep each PR independently reviewable; Workbench delivery need not wait for the model. |
| In the same D13 cycle                                   | Run the unchanged deterministic control and evaluate the proposed worker on representative unseen permitted inputs; compare usefulness and total review/rework/cost.                                                                        | Grounded claims, conflicts, abstention, correction, stale consent and result isolation. Failed quality gates narrow the task or retain the deterministic path; model use alone is not acceptance.                                                                   |
| As soon as its separate data boundary is approved       | Conduct an assisted customer preparation comparison, then extend to observed disposition/acceptance when evidence permits.                                                                                                                  | Agree cohort, baseline, named participants, custody/access/deletion, quality and full effort/cost capture. Customer learning does not wait for the model, D14 distribution, connectors or production writes.                                                        |
| After a valid baseline and person-selected intervention | Test one proposed process improvement on a comparable batch and review the next constraint.                                                                                                                                                 | Retain failed/open work and all five distinct proof measures. A person chooses the improvement; synthetic success, a model output and returning customers do not by themselves establish PMF.                                                                       |

The canonical [MVP completion and investigation boundary](docs/product/workflow-discovery.md#remaining-d13-mvp-completion)
defines the task and product acceptance checks. Keep D14 distribution, D15 broader
provider/model routing, D16 Operational CI, D17 read-only connectors, D18 connected
shadow operation, D19 enterprise coordination and D20 production actions in their
existing stages. No autonomous scheduler, general agent harness or parallel Case model.

Within the Workbench design, distinguish reusable workflow setup from daily work.
Inventory which steps are template/source/permission setup, which bind this record's
current evidence, and which require consequential judgment. Preserve current explicit
invocations, fresh review/publication and exact retries. Any proposal to reuse consent,
start work automatically or change permissions must state its separate contract and
approval boundary; collapsing a panel does not change authority.
