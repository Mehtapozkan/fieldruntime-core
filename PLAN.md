...
or a separately approved bounded change is recorded. The proposed single D13
investigation task below is distinct from broad D15 routing; its model/data boundary
still needs approval before implementation.
...
[Proposed D-040 investigation task](docs/architecture/d13-bounded-investigation-task.md)
requires approval before provider integration. Stop this build at the Workbench handoff.
...
| Next                                                    | Review D-039 API evidence and dependency before any normal protected merge. Proposed D-040 specifies the bounded model task; implementation awaits separate approval.                                                  | BR1–BR8: same-record proof, exact authority, reported disposition, independent observation, separate business acceptance, remaining commitments and reversal. No model dependency or complete Case closure.                                                         |
| Immediately after the API                               | Review the implemented minimum result controls and reconciled progress in the existing Workbench. Implement the bounded investigation worker alongside this presentation slice once its separate boundary is approved. | Use the same result contracts, exact receipts and source associations. Demonstrate useful work, material uncertainty, next responsible role/action and current result state. Keep each PR independently reviewable; Workbench delivery need not wait for the model. |
