...
### Profile-bound v0 command defaults

These values complete the existing engine's required fields; D9-B must not infer
classification or severity from the invoice amount, free text, actor claims or
imported priority. The fixed synthetic profile supplies:

| Required field / generated binding | Value or deterministic source                                                                                                                                                                                                                                                                                                                                                                                      |
| ---------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Tenant seed                        | `id: tenant_intake_demo`, `name: Invoice intake synthetic evaluation`, `status: active`, `data_region: local`, `retention_policy_id: retention_intake_synthetic_v1`. The retention ID is a synthetic profile label, not an implemented retention service.                                                                                                                                                          |
| Intake namespace / entity scopes   | `intake_scope_id: scope_invoice_disputes`; north maps to `scope_entity_north`, south to `scope_entity_south`. New Case and WorkEvent scopes use only the selected entity's scope; existing Case attachment still requires that subset. The server's synthetic intake context can access both approved entities.                                                                                                    |
| Workflow seed                      | Exact existing ECC metadata with `status: shadow`: ID `workflow_ecc_v0_1_0`, workflow `customer_escalation_commitment_control`, version/eval suite `0.1.0`, graph `ecc_decision_graph_v0`, policy refs `[policy_customer_comms_v3]`, effective from `2026-08-26T00:00:00.000Z` with source timezone `UTC`. These are references, not policy enrollment or activation.                                              |
| Initial Case fields                | Stable root-derived ID as below; tenant/workflow/scopes from profile; **`severity: medium`**; **`owner_identity_id: null`** for this first profile (no imported identity mapping). Customer reference is the scoped reviewed source claim, issue fingerprint the scoped Case root. Omit optional deadlines, parent/related Cases and effective-to metadata. State/version/created/updated times stay engine-owned. |
| WorkEvent classification           | **`classification: internal`**, fixed for synthetic material. Actual customer classification remains unresolved and cannot inherit this default through a real-data approval.                                                                                                                                                                                                                                      |
| WorkEvent identity / idempotency   | `id = work_event_` plus M's 64 hex characters; `source = fieldruntime_intake`; `source_event_id = intake:` plus M; `idempotency_key = intake-event:` plus M. Required classification/idempotency are supplied even though schema-only validation does not require both.                                                                                                                                            |
| Command attribution / retry        | Server actor `identity_intake_operator`; input selection key is nonempty and at most 128 characters. Core key is `intake-command:` plus H(selection key); correlation is `intake-correlation:` plus the same hash. No caller actor/correlation override. Optional causation omitted; intake predecessor lineage remains in retained material/receipt.                                                              |
| Reviewed payload / occurrence      | `content_hash = H(review_material)`, opaque scoped reference to that material, `event_type: intake.material_reviewed`, one checked server review time and `source_timezone: UTC`. Create sends it as `trigger_event`; attach as `work_event`. Command tenant/target/expected C come from the reviewed, server-checked binding.                                                                                     |

Attachment must not overwrite existing severity or ownership. The synthetic
`medium` value is an initial triage default, not a confirmed risk judgment. A future
real-data profile needs an explicitly approved classification/priority mapping;
source claims alone cannot supply it.

Raw CSV bytes may contain both entities even when one row is attached to one Case.
Artifacts retain the preparation profile's full scope set. Source download/export
must check that full artifact scope, not merely the narrower Case citation; a
reference never grants access. D9-B's labelled synthetic context has that access;
no selective row disclosure or production multi-user access control is implied.
...
- **Reviewed material key M:** H(profile ID/version, K, V, sorted support document identities/byte hashes,
...
in the first profile; a later identity mapping would require a reviewed profile change. Show reported
...
actor, `classification = internal`, deterministic event/key fields, scoped `payload_ref = intake://<commit-material-id>` and computed content hash.
...
Case ID/seed from the reviewed scoped identity and the fixed defaults above. Attach requires the exact Case ID.
...
| A8 Invalid / unsupported / injected input | Broken CSV, wrong encoding/limit/media signature, DOCX/ZIP, scope escape, URL fetch, claimed actor/role/policy or “approve credit” document instruction | Fatal preparation errors retain no bytes/Case changes; semantic invalid rows remain noncommittable with findings. Client classification/severity/identity overrides are denied; valid generated commands contain every required profile field. Unknown boundary properties denied. Document text remains inert evidence, actor claims never become identities, source hashes/ranks never grant authority. |
